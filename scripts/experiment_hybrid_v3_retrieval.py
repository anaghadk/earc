"""
scripts/experiment_hybrid_v3_retrieval.py — Final Controlled Hybrid-v3 Retrieval Experiment.

Evaluates 3 retrieval configurations on the 10 benchmark queries (5 NQ + 5 TriviaQA):
  1. Dense Top-10 (and up to Top-20)
  2. Current RRF 50/50
  3. Hybrid-v3: Max-Rank + Agreement Boost (agreement_bonus=0.005)

Formula for Hybrid-v3:
  dense_comp = 1 / (60 + dense_rank) if in dense_top50 else 0.0
  bm25_comp = 1 / (60 + bm25_rank) if in bm25_top50 else 0.0
  base_score = max(dense_comp, bm25_comp)
  final_score = base_score + 0.005 if in_both else base_score
"""
...
def hybrid_v3_max_rank_agreement(
    dense_ids: List[int],
    bm25_ids: List[int],
    agreement_bonus: float = 0.005,
    k: int = 60,
) -> List[int]:
    """
    Hybrid-v3: Max-Rank + Agreement Boost.
    base_score = max(dense_comp, bm25_comp)
    final_score = base_score + agreement_bonus (if in both) else base_score
    """
    dense_ranks = {cid: rank_0 + 1 for rank_0, cid in enumerate(dense_ids)}
    bm25_ranks = {cid: rank_0 + 1 for rank_0, cid in enumerate(bm25_ids)}

    all_cids = set(dense_ids).union(set(bm25_ids))
    scores: Dict[int, float] = {}

    for cid in all_cids:
        d_comp = 1.0 / (k + dense_ranks[cid]) if cid in dense_ranks else 0.0
        b_comp = 1.0 / (k + bm25_ranks[cid]) if cid in bm25_ranks else 0.0

        base_score = max(d_comp, b_comp)
        in_both = (cid in dense_ranks) and (cid in bm25_ranks)
...
def run_experiment():
    print("=" * 70)
    print("CONTROLLED HYBRID-V3 RETRIEVAL EXPERIMENT (Max-Rank + Agreement)")
    print("=" * 70)
    ...
    out_dir = Path(f"outputs/hybrid_v3_retrieval_{timestamp}")
    ...
    methods = [
        ...
        ("Hybrid-v3 (Max-Rank + Agreement)", "hybrid_v3"),
    ]
    ...
    for q_idx, ex in enumerate(all_examples, 1):
        ...
        for m_name, m_key in methods:
            ...
            elif m_key == "hybrid_v3":
                fused = hybrid_v3_max_rank_agreement(dense_cids_50, bm25_cids_50, agreement_bonus=0.005, k=60)
                fuse_lat = time.time() - t_fuse_start
                top_cids = fused[:20]
                m_lat = dense_lat + bm25_lat + fuse_lat
            else:
            ...
            prefix = m_name.replace(" ", "_").replace("/", "_").replace("-", "_").replace("(", "").replace(")", "")
        ...
        print(f"  Q{q_idx:02d} completed: {query[:50]}...")

    # Summary table calculation
    summary_data = []
    print("\n" + "=" * 70)
    print("HYBRID-V3 EXPERIMENT SUMMARY TABLE (N=10 queries)")
    print("=" * 70)
    print(f"{'Method':<34} | {'Hit@1':<7} | {'Hit@5':<7} | {'Hit@10':<7} | {'Hit@20':<7} | {'Avg Latency':<12}")
    print("-" * 70)
    ...
    for m_name, _ in methods:
        ...
        print(
            f"{m_name:<34} | {mean_h1*100:6.1f}% | {mean_h5*100:6.1f}% | {mean_h10*100:6.1f}% | {mean_h20*100:6.1f}% | {avg_lat*1000:9.1f} ms"
        )
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# Controlled Hybrid-v3 Retrieval Experiment Report\n\n")
        f.write(f"- **Timestamp**: {timestamp}\n")
        f.write(f"- **Queries Evaluated**: 10 (5 NQ + 5 TriviaQA)\n")
        f.write(f"- **Method Comparison**: Dense vs Current RRF 50/50 vs Hybrid-v3 (Max-Rank + Agreement Boost)\n\n")
        ...
        f.write("\n## Per-Question Hit@10 Results\n\n")
        f.write("| # | Dataset | Question | Dense Hit@10 | Current RRF 50/50 Hit@10 | Hybrid-v3 Hit@10 |\n")
        f.write("| :--- | :--- | :--- | :--- | :--- | :--- |\n")
        for row in per_question_rows:
            f.write(
                f"| Q{row['q_idx']} | {row['dataset'].upper()} | {row['question']} | "
                f"{row['Dense_Top_10_hit@10']} | {row['Current_RRF_50_50_hit@10']} | "
                f"{row['Hybrid_v3_Max_Rank_+_Agreement_hit@10']} |\n"
            )
